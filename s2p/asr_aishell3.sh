...
MANIFEST_DIR=${WORK_DIR}/manifest/p2u/aishell3
...
LABEL=ph_g2puv2
#LABEL=ph_randlex
MODEL=wav2vec_small-ft-aishell3-$LABEL

W2V=${WORK_DIR}/outputs/s2p/${MODEL}-s2024/checkpoint_last.pt
# W2V=${WORK_DIR}/../s2p/outputs/${MODEL}/checkpoint_best.pt

# W2V=${WORK_DIR}/../s2p/outputs/${MODEL}/checkpoint_last.pt
...
SPLIT=dev
DECODE='viterbi'
results_dir=results/asr_${MODEL}_${DECODE}_v2last
POST_PROCESS=letter
...
~/software/SCTK/bin/sclite -i wsj -r ${results_dir}/ref.units-checkpoint_last.pt-${SPLIT}.txt  trn -h ${results_dir}/hypo.units-checkpoint_last.pt-${SPLIT}.txt trn | tee ${results_dir}/${SPLIT}.uer
