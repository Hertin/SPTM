...
@dataclass
class RoBERTaFtConfig(FairseqDataclass):
...
@dataclass
class RoBERTaFtCtcConfig(RoBERTaFtConfig):
    blank_weight: float = 0
    blank_mode: str = "add"


@register_model("roberta_ft", dataclass=RoBERTaFtCtcConfig)
class RoBERTaFt(BaseFairseqModel):
    def __init__(self, cfg: RoBERTaFtCtcConfig, roberta_encoder: BaseFairseqModel, target_dictionary: Dictionary):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: RoBERTaFtCtcConfig, task: FairseqTask):
